    ...
    if m is None:
        ...
        for x in m:
            if o.query == "":  # Some websites just have the file name E.G. XYZ-00001.ts
                print "Downloading: " + reconstructURL + x
                testfile.retrieve(reconstructURL + x, "video-" + str(y) + ".ts")  # retrieve files
            else:  # Some websites include some form of token that is necessary for downloading E.G. http://XYZ.com/seg-1.ts?e=ABC
                print "Downloading: " + reconstructURL + x + '?' + o.query
                testfile.retrieve(reconstructURL + x + '?' + o.query, "video-" + str(y) + ".ts")  # retrieve files
            y = y + 1
        ...
        for x in m:
            print "Downloading: " + x
            testfile.retrieve(x, "video-" + str(y) + ".ts")  # retrieve files
            y = y + 1
