...
def combineTS():
    ...
    if OSplatform == 'Windows':
        ...
        if (outputFileName == "" and os.path.isfile("CompleteVideo.ts") == False):
        ...
        elif (outputFileName == "" and os.path.isfile("CompleteVideo.ts") == True):
            rndnum = random.randint(2, 10000)
            if (os.path.isfile("CompleteVideo" + str(rndnum)) == False):
                subprocess.call('copy /b videoA.ts + videoB.ts + videoC.ts + videoD.ts + videoE.ts + videoF.ts + videoG.ts + videoH.ts + videoI.ts + videoJ.ts + videoK.ts + videoL.ts + videoM.ts + videoN.ts CompleteVideo' + str(rndnum) + '.ts',shell=True)
            elif (os.path.isfile("CompleteVideo" + str(rndnum)) == True):
                print "Duplicate File Name Randomizing attempt 2"
                rndnum += random.randint(1,10)
                if (os.path.isfile("CompleteVideo" + str(rndnum)) == False):
                    subprocess.call('copy /b videoA.ts + videoB.ts + videoC.ts + videoD.ts + videoE.ts + videoF.ts + videoG.ts + videoH.ts + videoI.ts + videoJ.ts + videoK.ts + videoL.ts + videoM.ts + videoN.ts CompleteVideo' + str(rndnum) + '.ts',shell=True)
                elif (os.path.isfile("CompleteVideo" + str(rndnum)) == True):
                    print "Duplicate File Name Randomizing attempt 3 [FINAL ATTEMPT] because if a filename is guessed again go play lotto"
                    rndnum += random.randint(1, 10)
                    subprocess.call('copy /b videoA.ts + videoB.ts + videoC.ts + videoD.ts + videoE.ts + videoF.ts + videoG.ts + videoH.ts + videoI.ts + videoJ.ts + videoK.ts + videoL.ts + videoM.ts + videoN.ts CompleteVideo' + str(rndnum) + '.ts', shell=True)
        else:
        ...
        if (outputFileName == "" and os.path.isfile("CompleteVideo.ts") == False):
            subprocess.call("cat *.ts >> CompleteVideo.ts")
        elif (outputFileName == "" and os.path.isfile("CompleteVideo.ts") == True):
            rndnum = random.randint(2, 10000)
            if (os.path.isfile("CompleteVideo" + str(rndnum)) == False):
                subprocess.call(["cat *.ts >> CompleteVideo" + str(rndnum) + ".ts"],shell=True)
            elif (os.path.isfile("CompleteVideo" + str(rndnum)) == True):
                print "Duplicate File Name Randomizing attempt 2"
                rndnum += random.randint(1,10)
                if (os.path.isfile("CompleteVideo" + str(rndnum)) == False):
...
def start():
    ...
    try:
        ...
        counter = Value('i', 0)
        p = Pool(initializer=init, initargs=(counter,), processes=2)
...
def readfile():
    global userInputURL
    starterTime = datetime.datetime.now()
    with open('ripTS-downloadlist.txt', 'r') as myfile:
        for line in myfile:
            # Confirm the first line
            print line
            # Call Functions to d/l and combine
            userInputURL = line
            start()
    endingTime = datetime.datetime.now()
    delta = endingTime - starterTime
    print "ripTS was successful Enjoy!"
    print "ripTS took for the entire download list: ", delta
